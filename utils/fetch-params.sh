...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/FuertecoinParams"
else
    PARAMS_DIR="$HOME/.fuertecoin-params"
fi
...
DOWNLOAD_URL="https://github.com/gitfuec/fuertecoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with FUEC_DISABLE_SOMETHING=1
FUEC_DISABLE_WGET="${FUEC_DISABLE_WGET:-}"
#FUEC_DISABLE_IPFS="${FUEC_DISABLE_IPFS:-}"
FUEC_DISABLE_CURL="${FUEC_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$FUEC_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$FUEC_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$FUEC_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Fuertecoin parameters!
...
main() {
    ...
    cat <<EOF
FUEC - fetch-params.sh

This script will fetch the Fuertecoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Fuertecoin SNARK parameters. Note that it is

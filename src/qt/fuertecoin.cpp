...
{
    return QCoreApplication::translate("fuertecoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in fuertecoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in fuertecoin.qrc)
...
/** Class encapsulating Fuertecoin Core startup and shutdown.
...
/** Main Fuertecoin application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // fuertecoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Fuertecoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Fuertecoin Core - \""), false, OptionsCategory::GUI);
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(fuertecoin);
    Q_INIT_RESOURCE(fuertecoin_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse fuertecoin.conf
    ...
    // fuertecoin: links repeatedly have their payment requests routed to this process:

...
const char * const BITCOIN_CONF_FILENAME = "fuertecoin.conf";
const char * const BITCOIN_PID_FILENAME = "fuertecoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "fuertecoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Fuertecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Fuertecoin
    // Mac: ~/Library/Application Support/Fuertecoin
    // Unix: ~/.fuertecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Fuertecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Fuertecoin";
#else
    // Unix
    return pathRet / ".fuertecoin";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty fuertecoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Fuertecoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Fuertecoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Fuertecoin Core developers";
    }
